...
use_random <- TRUE   #To external analyst set to FALSE for Real Data----
...
build_gt_referral_table <- function(df_long, use_random = TRUE) {
...
# --- 4. Call the function ---
arm_referral_summary <- build_gt_referral_table(df_long, use_random = use_random)
...
arm_outcome_summary <- arm_joined %>%
  tbl_summary(
    by = arm_random, # To external analyst: Replace with arm for real data----
...
ae_injury_long <- ae_df %>%
  ...
  filter(redcap_repeat_instance == 2 | !str_detect(redcap_event_name, "Enrollment")) %>%
  mutate(Event = ae_cat) %>%
  select(record_id, arm_random, Event)
