...
# introduce radnomized arm for blinding
    mutate(
    arm_random = sample(
      rep(c("control (Randomized)", "intervention (Randomized)"), length.out = n())
    )
  )
...
use_random <- TRUE # To external analyst; Replace with FALSE for real data---- 

df_long <- referral_df %>%
    # keep only the risk_* + referral_accept columns you need
    select(record_id, arm_random, starts_with("screened_"), referral_accept)%>%
    # To external analyst replace arm_random with Arm for real data---- 
    pivot_longer(
...
arm_summary_tbl <- df_long %>%
  group_by(risk_col, .data[[group_var]]) %>%
  reframe(
    n = n(),
  ...
  mutate(
  arm_clean = case_when(
    str_detect(.data[[group_var]], regex("control", ignore_case = TRUE)) ~ "control",
    str_detect(.data[[group_var]], regex("intervention", ignore_case = TRUE)) ~ "intervention"
  )
)


control_label <- if (use_random) "Control (Randomized)" else "Control"
intervention_label <- if (use_random) "Intervention (Randomized)" else "Intervention"


arm_referral_summary <- arm_summary_tbl %>%
  mutate(
    uptake = if_else(n > 0, (n_accepted / n) * 100, 0)
  ) %>%
  select(Condition, arm_clean, n, n_accepted, uptake) %>%
  pivot_wider(
    names_from = arm_clean,
    values_from = c(n, n_accepted, uptake),
    names_glue = "{.value}_{arm_clean}",
    values_fill = 0
  ) %>%
  gt() %>%
  tab_header(
