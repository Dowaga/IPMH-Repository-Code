...
arm_ids <- ppw_rct_df %>%
    filter(visit_type == "14 Weeks") %>% 
    select(clt_ptid, clt_study_site, arm)
...
arm_ids <- arm_ids %>% 
    left_join(outcome_arm, by = "clt_study_site") %>%
    mutate(
        arm_random = factor(arm_random,
                            levels = c("Control - randomly generated",
                                       "Intervention - randomly generated"))
...
arm_outcome_summary <- arm_joined %>%
    tbl_summary(
        by = arm_random, # External analyst please switch to arm for real data----
...
ae_summary <- ae_bin %>%
    tbl_summary(
        by = Arm, # External analyst please switch to arm_group----
