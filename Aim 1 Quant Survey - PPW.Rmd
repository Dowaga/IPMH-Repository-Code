...
#IPMH
```{r IPMH barriers to Screning, echo=F, message=FALSE, warning=FALSE}
ipmh_ppw <- ppw_data %>% 
...
gglikert(ipmh_screening_barriers,
         ...
         labels_hide_below = 0.05,
         width = 0.5)+
    labs(title = "Impact of IPMH Barriers to Screening",
         caption = "Note: n = 190") +  # Add a footnote
    theme(plot.caption = element_text(hjust = 0.5),
          plot.title = element_text(hjust = 0.5),)  # Center the footnote
...
```{r IPMH Barrier to PM+, echo=FALSE, message=FALSE, warning=FALSE}
ip_pm_barr <- ppw_data %>% 
