...
# read data
data <- read.csv("C:/Users/DAMARIS/UW/ngumbau - IPMH study/Data/4. Aim 1 quantitative data/Aim1_quant_data_2024-10-14.csv")
...
df <- ppw_demo %>% 
    select(age, women_service___4, women_service___2, women_service___1,
...
screen_ppw <- data  %>%
  dplyr::select(record_id, starts_with("scr_diff_"), screening_access, screening_strategy_other, screening_complete, screening_times, starts_with("scr_barrier")) %>% 
    mutate(across(
        starts_with("scr_barrier"),  
        ~ case_when(
            str_detect(., "Not a challenge at all") ~ "Not a challenge at all",
            str_detect(., "Makes it a little hard") ~ "Makes it a little hard",
            str_detect(., "Makes it hard") ~ "Makes it hard",
            str_detect(., "Makes it very hard") ~ "Makes it very hard",
            str_detect(., "Makes it impossible") ~ "Makes it impossible",
            str_detect(., "I don't know") ~ "I don't know",
            TRUE ~ NA_character_  # Use NA_character_ instead of NA_real_
        )
    )) %>% 
    mutate(across(
        starts_with("scr_diff_"),  
        ~ case_when(
            str_detect(., "Major negative impact") ~ "Major negative impact",
            str_detect(., "Some negative impact") ~ "Some negative impact",
            str_detect(., "No impact ") ~ "No impact",
            str_detect(., "Some positive impact") ~ "Some positive impact",
            str_detect(., "Major positive impact") ~ "Major positive impact",
            str_detect(., "I don't know") ~ "I don't know",
            TRUE ~ NA_character_  # Use NA_character_ instead of NA_real_
        )
    ))
...
freq_bar_df <- data %>% 
  ...
  mutate(across(where(is.numeric), ~ na_if(., 99))) %>% 
    mutate(across(
        starts_with("freq_barr_"),  
        ~ case_when(
            str_detect(., "Never") ~ "Never",
            str_detect(., "Rarely") ~ "Rarely",
            str_detect(., "Occasionally ") ~ "Occasionaly",
            str_detect(., "Frequently") ~ "Frequently",
            str_detect(., "Always") ~ "Always",
...
geom_text(aes(label = ifelse(percentage > 5, paste0(round(percentage, 1), "%"), "")), 
  ...
  theme(
    ...
    legend.text = element_text(size = 10),
    legend.position = "bottom",
    legend.justification.left = "right",
        axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
    axis.title.x = element_text(size = 15),
  ) +
    scale_x_discrete(labels = barrier_labels) +
...
screen_barriers_fre <- ipmh_ppw %>% 
    ...
    mutate(across(
        ...
        ~ case_when(
            str_detect(., "Never") ~ "Never",
            str_detect(., "Rarely") ~ "Rarely",
            str_detect(., "Occasionally ") ~ "Occasionally",
            str_detect(., "Frequently") ~ "Frequently",
            str_detect(., "Always") ~ "Always",
            str_detect(., "I don't know") ~ "I don't know",
            TRUE ~ NA_character_  # Use NA_character_ instead of NA_real_
...
tele_barr_df <- data  %>%
    ...
    mutate(across(
        ...
        ~ case_when(
            str_detect(., "Never") ~ "Never",
            str_detect(., "Rarely") ~ "Rarely",
            str_detect(., "Occasionaly ") ~ "Occasionaly",
            str_detect(., "Frequently") ~ "Frequently",
            str_detect(., "Always") ~ "Always",
            str_detect(., "I don't know") ~ "I don't know",
            TRUE ~ NA_character_  # Use NA_character_ instead of NA_real_
...
ggplot(tele_barr_long, aes(x = Barrier, y = percentage, fill = Response)) +
  geom_bar(stat = "identity", position = "fill", width = 0.5) +  # Stacked bar chart with fill
geom_text(aes(label = ifelse(percentage > 5, paste0(round(percentage, 1), "%"), "")), 
            position = position_fill(vjust = 0.5), size = 3, color = "black") +  # Adjusted label size
  scale_fill_brewer(palette = "RdYlBu", direction = -1) +  
  scale_y_continuous(labels = scales::percent) +  
  labs(
    x = NULL, 
...
geom_text(aes(label = ifelse(percentage > 5, paste0(round(percentage, 1), "%"), "")), 
  ...
  theme(
    ...
    legend.justification.left = "left",
    axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
    xis.title.x = element_text(size = 15),
  ) +
  scale_x_discrete(labels = strategy_label) +  # Apply custom labels for the strategies
  coord_flip() 
```
