...
## Retention
```{r Retention, echo=FALSE, message=FALSE, warning=FALSE}
source("RCT Retention.R")
wk6_retention_summary
```
...
none2_pm_survey <- pm_survey_df %>% 
    filter(pm_ptid%in% c(21177049, 21031052, 21010085,21094053,21073077, 21229516))
...
# Filter ptids with Post session but without session 5 
no_post_intervention <- pm_survey_df %>%
  group_by(pm_ptid) %>%
  filter(any(pm_session == "Post-intervention PSYCHLOPS measurement") & !any(pm_session == "Session 5 content"))%>%
  filter(pm_session == "Post-intervention PSYCHLOPS measurement")
