---
title: "CHV-NEO: Community-based digital communication to support neonatal health"
...
data_dir <- file.path(rctact_filepath, "/Data Management/Processed Study Data") # Processed data from R

data_freeze <- fileSnapshot(file.path(rctact_filepath,"eCHISdata/3_eCHIS_data_pull/data/tabular/archive"))
...
Report prepared by: Lincoln Pothan
...
vars2 <- c("Integrate the NEO interactive SMS intervention into routine community health promoter (CHP) workflow through the eCHIS and pragmatically evaluate its clinical, service delivery, and implementation outcomes", 
...
```{r timeline_prenum, echo=FALSE}
source("enroll_linegraph_ppw.R")

```

## 1.1 Study Timeline Since Initiation:
-   Apr 15, 2021: RO1 Start date
-   Jun 21, 2021: Initial IRB approval
-   Mar 15, 2022: Initial ERC approval
-   May 23, 2023: 1st DSMB meeting
-   Nov 06, 2023: eCHIS user training initiation
-   Dec 07, 2023: eCHIS household and pregnancy registration initiation
-   Jan 10, 2024: Sub cohort recruitment initiation
-   `r sub_enroll25_date`: Sub cohort 25% of planned enrollment
-   `r sub_enroll50_date`: Sub cohort 50% of planned enrollment
-   `r sub_enroll75_date`: Sub cohort 75% of planned enrollment
-   `r sub_enroll100_date`: Sub cohort 100% of planned enrollment
```{r timeline_prenum_2, echo=FALSE}
source("morcohort_linegraphs.R")
```
-   `r enroll25_date`: Mortality cohort 25% of planned target
...
```{r source_prenum, echo=FALSE}
source("weekly_consort.R")
```
CHPs began enrolling households into eCHIS on Dec 07, 23. `r all_echispreg` women have been registered as pregnant in eCHIS, of which `r livebirths` are known to have delivered live births. This achieves `r livebirths_per`% of our mortality cohort target. Of these, `r infantstatus_28days_y` of those have reported an infant status past 28 days of life.

We began recruitment into the sub-cohort on Jan 10, 24. `r enroll_ppw` participants have been recruited into the subcohort meeting `r enroll_ppw_per`% of our subcohort enrollment target. Of these, `r all_followup_6wk` (`r all_followup_6wk_per`%) have completed follow-up.

In general, enrollment has proceeded well despite delays in launching and slow initial household registration. A series of national healthcare worker strikes (Mar 12, 2024 and May 8, 2024), flooding (Apr 23, 2024 and May 12, 2024), and ongoing national protests beginning Jun 18, 2024 have slowed data collection in the larger mortality cohort.
...
From `r anc_attendance` ANC visits of non-unique clients at recruitment sites, `r screeningnum` were assessed for eligibility. `r total_excluded` potential participants were excluded, leaving `r enroll_ppw` eligible participants enrolled across intervention and control clusters. `r all_followup_6wk` participants have completed follow-up to 6 weeks potspartum.



```{r consort_sub}
source("weekly_consort.R")
weekly_consort_subcohort
