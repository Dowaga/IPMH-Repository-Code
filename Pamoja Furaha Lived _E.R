...
pamoja_furaha_age <- demographics_df %>% 
    ...
    select(clt_ptid, clt_date, clt_study_site, dem_age, dem_maritalstat)
...
# PPW HIV Status
hiv_status <- ppw_rct_df %>%
    filter(visit_type == "Enrollment")%>% 
    select(clt_ptid, med_pastdiag___2)

# Step 3: hiv status + pamoja_furaha-------------------------------------
pamoja_furaha <- pamoja_furaha %>% 
...
# --------------------------------------------------------------------
# Filter and arrange your dataset
pamoja_furaha_linelist <- pamoja_furaha_final %>%
    arrange(dem_age) %>%
    filter(referred_to == "PM+") %>%
    filter(completed_visit == "Yes") %>% 
    rename(hiv_status = med_pastdiag___2) %>% 
    mutate(hiv_status = case_when(
        hiv_status %in% c("Checked") ~ "+ve",
        hiv_status %in% c("Unchecked") ~ "-Ve"))  


# Save to Excel
write_xlsx(
    pamoja_furaha_linelist,
    "C:/Users/hp/OneDrive/Desktop/IPMH-Repository-Code/Pamoja Furaha/Pamoja_Furaha_linelist.xlsx"
)
