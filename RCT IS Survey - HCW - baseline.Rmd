...
#compare
setdiff(consent_id$screen_id, survey_id$pt_id) #0 IDs in the consenting database are not in the survey database
setdiff(survey_id$pt_id, consent_id$screen_id) #1 ID in the survey database is not in the consenting database
# 24140190 this is in the wrong arm as well.

rct_hcw %>% count(pt_id) %>% filter(n > 1) #0

# First and last date for Old Batch
Dates_summary <- rct_hcw %>%
     arrange(collect_time) %>%
    reframe(
    start_date = min(collect_time, na.rm = TRUE),
    end_date = max(collect_time, na.rm = TRUE)
  )
```
...
is_survey_df %>% 
    ...
    gt() %>%
    tab_header(
        title = "Number of Participants by Arm and Facility")
```
...
# reverse coding for some items: oric_demand_2, oric_demand_3, oric_context_2, oric_capacity_2, oric_flex_1
hcw_readiness_df <- hcw_readiness_df %>%
    mutate(across(c(oric_demand_2, oric_demand_3, oric_context_2, oric_capacity_2, oric_flex_1),
                  ~6 - .)) 
...
# Organizational readiness - Facility in-charge

```{r in-charge, echo=FALSE, message=FALSE, warning=FALSE}
incharge_readiness_df <- is_survey_df %>% 
        filter(pt_type == "Facility-in-charge: 23") %>% 
    select(pt_id, facility_id, pt_type, arm, ends_with("_fic"))
...
# reverse coding for some items: oric_demand_2, oric_demand_3, oric_context_2, oric_capacity_2, oric_flex_1
incharge_readiness_df <- incharge_readiness_df %>%
    mutate(across(c(oric_demand_2_fic, oric_demand_3_fic, oric_context_2_fic, oric_capacity_2_fic, oric_flex_1_fic),
                  ~6 - .))
...
```{r, continuous in-charge, echo=FALSE, message=FALSE, warning=FALSE}
#show the mean score of each domain by arm using tbl_summary
...
```{r, binary in-charge, echo=FALSE, message=FALSE, warning=FALSE}
#making the scores binary
incharge_readiness_df <- incharge_readiness_df %>% 
    mutate(across(c(oric_change_commitment, oric_change_efficacy, oric_capacity, oric_flex, oric_org),
                  ~case_when(. >= 4 ~ "High",
