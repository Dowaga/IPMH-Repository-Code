...
#read data
data <- read.csv("~/Desktop/uw/IPMH/data analysis/IPMHAim1Quantitative_DATA_2024-07-23_2308.csv")
```
...
ggplot(stats, aes(x = mean_importance, y = mean_addressability)) +
  ...
  geom_text_repel(aes(label = barrier, color = barrier_detail), size = 6,
                  max.overlaps = 20) +
  guides(size = FALSE, color = guide_legend(override.aes = list(shape = 16, size = 6))) +
  theme_minimal() +
...
stats <- stats %>% 
  mutate(barrier_detail = dplyr::recode(barrier, 
                                  ...
                                  "stigma" = "Stigma: Stigmatization from the community",
                                   "workload" = "Workload: Having too many clients to see",
                                  "fear" =	"Fear: Women's fear of opening up to HCW",
                                  "drug" = "Drug: Limited drug supply to manage patients",
...
stats <- stats %>% 
  mutate(barrier_detail = dplyr::recode(barrier, 
                                        "reach" = "Reach: Lay providers do not reach all PPW coming to the facility",
                                        ...
                                        "flow" = "Flow: Complex flow from screening to getting PM+, leading to loss of patients",
...
ggplot(stats, aes(x = mean_importance, y = mean_addressability)) +
  ...
  theme(axis.text.y = element_text(size = 12),
        ...
        legend.text = element_text(size = 12),
        legend.direction = "vertical",
        legend.position = "bottom", # can change this to none
        plot.margin = margin(1, 1, 1, 1, "cm"))
...
stats <- stats %>% 
  mutate(barrier_detail = dplyr::recode(barrier, 
                                        ...
                                        "staff" = "Staff: Shortage of staff",
                                        "incentive" = "Incentive: Lack of financial incentives to motivate HCWs",
                                        "flow" = "Complex flow from screening to getting PM+, leading to loss of patients",
                                        "prefer" = "Prefer: Patients prefer to deal with one HCW as opposed to many",
                                        "confi" = "Confi: Fear of breach of confidentiality",
                                        "long" = "Long: Lengthening of patient visits",
                                        "partner" = "Partner: Lack of partner involvement"))
...
stats <- stats %>% 
  mutate(strategy_detail = dplyr::recode(strategy, 
                                         ...
                                         "incentive" = "Incentive: Provide financial incentives to HCWs",
                                         "chv" = "CHV: Involving community health promoters",
                                         "partner" = "Partner: Encourage male partner involvement",
                                         "info" = "Info: Provide IPMH information and reassurance at the triage desk to ensure confidentiality"))
...
stats <- stats %>% 
  mutate(barrier_detail = dplyr::recode(barrier, 
                                        ...
                                        "drug" = "Drug: Inadequate or lack of access to the medication prescribed",
                                        "setup" = "Setup: User challenges in setting up and using telepsychiatry",
                                        "internet" = "Internet: Internet connectivity for telepsychiatry",
                                        "port" = "Port: Portability of the device used for telepsychiatry",
                                        "gadget" = "Gadget: Lack of required gadgets to conduct telepsychiatry",
                                        "inte" = "Inte: Challenging process of integration with the care at the facility",
                                        "language" = "Language: Language barrier between women and psychiatrists",
                                        "confi" = "Confi: Fear of breach of confidentiality",
                                        "physical" = "Physical: Clients' preference to talk to psychiatrists in person instead of online"))
