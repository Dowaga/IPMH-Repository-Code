...
# -----------------------------
# 1) Original pipeline + provider_type
# -----------------------------
ipmh_hcw <- data2 %>% select(record_id, provider_type, starts_with("ipmh_"))
ipmh_screen <- ipmh_hcw %>% select(record_id, provider_type, starts_with("ipmh_scre_"))

ipmh_screen_barrier <- ipmh_screen %>% 
  select(record_id, provider_type, matches("bar|add"), ipmh_scre_barrier_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))
...
# -----------------------------
# 2) Stats: mean coordinates + SD consensus (conservative)
# -----------------------------
stats <- long_data %>%
  group_by(barrier, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
    sd_value   = sd(value_1, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(names_from = type, values_from = c(mean_value, sd_value)) %>%
  rename(
    mean_importance     = mean_value_bar,
    sd_importance       = sd_value_bar,
    mean_addressability = mean_value_add,
    sd_addressability   = sd_value_add
  ) %>%
  mutate(sd_consensus = pmax(sd_importance, sd_addressability))  # larger = lower consensus

# -----------------------------
# 3) Median cutoffs (median of item means = true median split of points)
# -----------------------------
cutoff_importance <- median(stats$mean_importance, na.rm = TRUE)
cutoff_addressability <- median(stats$mean_addressability, na.rm = TRUE)
...
# -----------------------------
# 5) Plot (no size legend; title notes provider_type coverage)
# -----------------------------
p_screen_all <- ggplot(stats, aes(x = mean_importance, y = mean_addressability)) +
  geom_point(aes(color = barrier_detail, size = sd_consensus), shape = 1) + 
  scale_x_continuous(breaks = 1:4, limits = c(1, 4)) +
  scale_y_continuous(breaks = 1:4, limits = c(1, 4)) +
  geom_hline(yintercept = cutoff_addressability, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = cutoff_importance, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_importance, y = 4,
           ...
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 1.2, y = cutoff_addressability,
           label = paste0("Cutoff: ", round(cutoff_addressability, 2)),
           vjust = -1, size = 3.5) +
  labs(title = "Panel A: Barriers",
    x = "Importance",
    y = "Addressability",
    color = "Barrier detail"
  ) +
  ...
  color = guide_legend(
    ...
    title.hjust = 0.5,
    ncol = 1, nrow=8,
    override.aes = list(shape = 16, size = 6)
  )
) +
  theme_minimal() + 
  theme(
...
### Companion table: per-barrier ratings, spread, and zone -----
# n and median[IQR] per barrier per axis (respects the same NA handling as the plot)
axis_summary <- long_data %>%
  filter(type %in% c("bar", "add")) %>%
  group_by(barrier, type) %>%
  summarise(
    n      = sum(!is.na(value_1)),
    median = median(value_1, na.rm = TRUE),
    q1     = quantile(value_1, 0.25, na.rm = TRUE),
    q3     = quantile(value_1, 0.75, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(
    names_from  = type,
    values_from = c(n, median, q1, q3)
  ) %>%
  rename(
    n_importance      = n_bar,      n_addressability      = n_add,
    median_importance = median_bar, median_addressability = median_add,
    q1_importance     = q1_bar,     q1_addressability     = q1_add,
    q3_importance     = q3_bar,     q3_addressability     = q3_add
  )

barrier_table <- stats %>%
  left_join(axis_summary, by = "barrier") %>%
  mutate(
    zone = case_when(
      mean_importance >= cutoff_importance &
        mean_addressability >= cutoff_addressability ~ "Priority (Go)",
      mean_importance >= cutoff_importance &
        mean_addressability <  cutoff_addressability ~ "Important but hard",
      mean_importance <  cutoff_importance &
        mean_addressability >= cutoff_addressability ~ "Easy but lower value",
      TRUE                                           ~ "Deprioritize"
    ),
    # flag barriers sitting on a cutoff line (zone is fragile)
    near_cutoff = abs(mean_importance - cutoff_importance) < 0.05 |
                  abs(mean_addressability - cutoff_addressability) < 0.05,
    # readable one-line label (strip the newlines from barrier_detail)
    barrier_label = gsub("\\s*\\n\\s*", " ", barrier_detail)
  ) %>%
  arrange(desc(mean_importance), desc(mean_addressability)) %>%
  ...
  transmute(
    ...
    zone,
    near_cutoff,
    importance      = sprintf("%.2f (%.2f)", mean_importance, sd_importance),
    addressability  = sprintf("%.2f (%.2f)", mean_addressability, sd_addressability),
    median_importance     = sprintf("%g [%g\u2013%g]", median_importance, q1_importance, q3_importance),
    median_addressability = sprintf("%g [%g\u2013%g]", median_addressability, q1_addressability, q3_addressability),
    sd_consensus    = round(sd_consensus, 2),
    n_importance,
    n_addressability
  )
...
#write.csv(barrier_table, "barrier_table_panelA.csv", row.names = FALSE)
```


```{r subgroup analysis}
# 1) Subgroup stats (means + SD consensus) --------------------------------------
stats_sub <- long_data %>%
  filter(!is.na(provider_type)) %>%
  ...
  mutate(sd_consensus = pmax(sd_importance, sd_addressability)) %>%
  mutate(
    barrier_detail = dplyr::recode(
      barrier, 
      "reach"     = "Reach: Lay providers do not reach\nall women coming to the facility",
      "workload"  = "Workload: Increased staff workload",
      "staff"     = "Staff: Shortage of staff",
      "incentive" = "Incentive: Lack of financial incentives to motivate providers",
...
# 2) Median cutoffs WITHIN subgroup (median of item means) ----------------------
cutoffs_sub <- stats_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_importance     = median(mean_importance, na.rm = TRUE),
    cutoff_addressability = median(mean_addressability, na.rm = TRUE),
    .groups = "drop"
  )

# 3) Plot helper ---------------------------------------------------------------
plot_screen_subgroup <- function(pt_label) {

  df <- stats_sub %>% filter(provider_type == pt_label)

  cx <- cutoffs_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_importance)
  cy <- cutoffs_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_addressability)

  ggplot(df, aes(x = mean_importance, y = mean_addressability)) +
    geom_point(aes(color = barrier_detail, size = sd_consensus), shape = 1) + 
    scale_x_continuous(breaks = 1:4, limits = c(1, 4)) +
    scale_y_continuous(breaks = 1:4, limits = c(1, 4)) +
    geom_hline(yintercept = cy, linetype = "dashed", color = "grey") +
    geom_vline(xintercept = cx, linetype = "dashed", color = "grey") +
    annotate("text", x = cx, y = 4,
             label = paste0("Cutoff: ", round(cx, 2)),
             hjust = -0.1, size = 3.5) +
    annotate("text", x = 1.1, y = cy,
             label = paste0("Cutoff: ", round(cy, 2)),
             vjust = -1, size = 3.5) +
    labs(
      title = paste0("", pt_label, ""),
      x = "Importance",
      y = "Addressability",
      color = "Barrier detail"
    ) +
...
# 4) Two subgroup plots --------------------------------------------------------
p_screen_lay <- plot_screen_subgroup("Lay providers")
p_screen_formal <- plot_screen_subgroup("Formally trained providers")

p_screen_subgroups <- ggarrange(
  p_screen_lay, p_screen_formal,
  ncol = 2,
  common.legend = TRUE,
  legend = "bottom"
)
...
# ggsave("Fig_screen_barriers_subgroups_1200px.png",
#         p_screen_subgroups,
#         dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
```
...
ipmh_screen_strategy <- ipmh_screen %>% 
  select(record_id, provider_type, matches("stra|diff"), ipmh_scre_strategy_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))

long_strat <- ipmh_screen_strategy %>%
  pivot_longer(
    cols = -c(record_id, provider_type, ipmh_scre_strategy_other),
    names_to = c("type", "strategy"),
    names_pattern = "ipmh_scre_(.*)_(.*)"
  ) %>% 
  mutate(
    value_1 = case_when(
      value == "I don't know" ~ NA_real_,
      value %in% c("Very hard", "Major negative impact") ~ 1,
      value %in% c("Somewhat hard", "Some negative impact") ~ 2,
      value %in% c("Neurtral", "No impact") ~ 3,
      value %in% c("Somewhat easy", "Some positive impact") ~ 4,
      value %in% c("Very easy", "Major positive impact") ~ 5,
      TRUE ~ NA_real_
    ),
    strategy = ifelse(strategy == "plan", "streamline", strategy)
  )

# 2) ALL participants: mean coords + SD consensus + median cutoffs -------------
stats_strat <- long_strat %>%
  group_by(strategy, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
    sd_value   = sd(value_1, na.rm = TRUE),
  ...
  rename(
    mean_feasibility = mean_value_stra,
    sd_feasibility   = sd_value_stra,
    mean_impact      = mean_value_diff,
    sd_impact        = sd_value_diff
  ) %>%
  mutate(sd_consensus = pmax(sd_feasibility, sd_impact))

cutoff_impact <- median(stats_strat$mean_impact, na.rm = TRUE)
cutoff_feasibility <- median(stats_strat$mean_feasibility, na.rm = TRUE)

# labels
stats_strat <- stats_strat %>%
  mutate(
    strategy_detail = dplyr::recode(
      strategy, 
      "nurse"      = "Nurse: Have the nurses screen at 1st contact",
      "room"       = "Room: Provide a specific area for \nscreening at the facility",
      "incentive"  = "Incentive: Provide financial incentives \nto motivate providers",
      "trainall"   = "Trainall: Train all providers",
...
p_screen_strategy_all <- ggplot(stats_strat, aes(x = mean_impact, y = mean_feasibility)) +
  geom_point(aes(color = strategy_detail, size = sd_consensus), shape = 1) +
  scale_x_continuous(breaks = 1:5, limits = c(4, 5)) +
  scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
  geom_vline(xintercept = cutoff_impact, linetype = "dashed", color = "grey") +
  geom_hline(yintercept = cutoff_feasibility, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_impact, y = 5,
           label = paste0("Cutoff: ", round(cutoff_impact, 2)),
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 4.1, y = cutoff_feasibility,
           label = paste0("Cutoff: ", round(cutoff_feasibility, 2)),
           vjust = -1, size = 3.5) +
  labs(title = "Panel B: Strategies",
       x = "Impact", y = "Feasibility", color = "Strategy detail") +
  geom_text_repel(aes(label = strategy, color = strategy_detail), size = 4) +
  guides(
  size = "none",
  color = guide_legend(
...
p_screen_strategy_all

fig1 <- ggarrange(
  p_screen_all, p_screen_strategy_all,
  nrow = 2,
  common.legend = F,
  legend = "right"
)
...
# ggsave("Fig1_1200px.png",
#         fig1,
#         dpi=400, width = 8.5, height = 8.5, limitsize = F, bg = "white")

# Companion table: per-strategy ratings, spread, and zone (Panel B) ------

# n and median[IQR] per strategy per axis
axis_summary_strat <- long_strat %>%
  filter(type %in% c("stra", "diff")) %>%
  group_by(strategy, type) %>%
  summarise(
    n      = sum(!is.na(value_1)),
    median = median(value_1, na.rm = TRUE),
  ...
  rename(
    n_feasibility      = n_stra,      n_impact      = n_diff,
    median_feasibility = median_stra, median_impact = median_diff,
    q1_feasibility     = q1_stra,     q1_impact     = q1_diff,
    q3_feasibility     = q3_stra,     q3_impact     = q3_diff
  )

strategy_table <- stats_strat %>%
  left_join(axis_summary_strat, by = "strategy") %>%
  mutate(
    zone = case_when(
      mean_impact >= cutoff_impact &
        mean_feasibility >= cutoff_feasibility ~ "Priority (Go)",
      mean_impact >= cutoff_impact &
        mean_feasibility <  cutoff_feasibility ~ "High impact but hard",
      mean_impact <  cutoff_impact &
        mean_feasibility >= cutoff_feasibility ~ "Easy but lower impact",
      TRUE                                     ~ "Deprioritize"
    ),
    near_cutoff = abs(mean_impact - cutoff_impact) < 0.05 |
                  abs(mean_feasibility - cutoff_feasibility) < 0.05,
    strategy_label = gsub("\\s*\\n\\s*", " ", strategy_detail)
  ) %>%
  arrange(desc(mean_impact), desc(mean_feasibility)) %>%
  mutate(rank = row_number()) %>%
  transmute(
    rank,
    strategy,
    ...
    zone,
    near_cutoff,
    impact       = sprintf("%.2f (%.2f)", mean_impact, sd_impact),
    feasibility  = sprintf("%.2f (%.2f)", mean_feasibility, sd_feasibility),
    median_impact      = sprintf("%g [%g-%g]", median_impact, q1_impact, q3_impact),
    median_feasibility = sprintf("%g [%g-%g]", median_feasibility, q1_feasibility, q3_feasibility),
    sd_consensus = round(sd_consensus, 2),
    n_impact,
    n_feasibility
  )
...
```{r subgroup analysis_strategy}
# 3) Subgroups: lay vs formal --------------------------------------------------
stats_strat_sub <- long_strat %>%
  filter(!is.na(provider_type)) %>%
  group_by(provider_type, strategy, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
    sd_value   = sd(value_1, na.rm = TRUE),
    .groups = "drop"
  ...
  mutate(
    strategy_detail = dplyr::recode(
      ...
      "incentive"  = "Incentive: Provide financial incentives to motivate providers",
      "trainall"   = "Trainall: Train all providers",
      "streamline" = "Streamline: Streamline workflow at the facility",
      "chv"        = "CHV: Involve community health promoters",
      "partner"    = "Partner: Encourage male partner involvement",
...
cutoffs_strat_sub <- stats_strat_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_impact      = median(mean_impact, na.rm = TRUE),
    cutoff_feasibility = median(mean_feasibility, na.rm = TRUE),
    .groups = "drop"
  )

plot_strategy_subgroup <- function(pt_label) {
  df <- stats_strat_sub %>% filter(provider_type == pt_label)
  cx <- cutoffs_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_impact)
  cy <- cutoffs_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_feasibility)

  ggplot(df, aes(x = mean_impact, y = mean_feasibility)) +
    geom_point(aes(color = strategy_detail, size = sd_consensus), shape = 1) +
    scale_x_continuous(breaks = 1:5, limits = c(4, 5)) +
    scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
    geom_vline(xintercept = cx, linetype = "dashed", color = "grey") +
    geom_hline(yintercept = cy, linetype = "dashed", color = "grey") +
    annotate("text", x = cx, y = 5,
             label = paste0("Cutoff: ", round(cx, 2)),
             hjust = -0.1, size = 3.5) +
    annotate("text", x = 4.1, y = cy,
             label = paste0("Cutoff: ", round(cy, 2)),
             vjust = -1, size = 3.5) +
    labs(title = pt_label, x = "Impact", y = "Feasibility", color = "Strategy detail") +
    geom_text_repel(aes(label = strategy, color = strategy_detail), size = 4) +
    guides(
  size = "none",
  color = guide_legend(
...
p_strategy_lay <- plot_strategy_subgroup("Lay providers")
p_strategy_formal <- plot_strategy_subgroup("Formally trained providers")

p_strategy_subgroups <- ggarrange(
  p_strategy_lay, p_strategy_formal,
  ncol = 2, common.legend = TRUE, legend = "bottom"
)

p_strategy_subgroups

# ggsave("Fig_screen_strategies_subgroups_1200px.png",
#         p_strategy_subgroups,
#         dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
```
...
ipmh_pm <- ipmh_hcw %>% select(record_id, provider_type, starts_with("ipmh_trt_"))

ipmh_pm_barrier <- ipmh_pm %>%
  select(record_id, provider_type, matches("bar|add"), ipmh_trt_barrier_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))

long_data_pm <- ipmh_pm_barrier %>%
  pivot_longer(
    cols = -c(record_id, provider_type, ipmh_trt_barrier_other),
    names_to = c("type", "barrier"),
...
# median split of the plotted points
cutoff_importance_pm <- median(stats_pm$mean_importance, na.rm = TRUE)
cutoff_addressability_pm <- median(stats_pm$mean_addressability, na.rm = TRUE)
...
stats_pm <- stats_pm %>%
  mutate(
    barrier_detail = dplyr::recode(
      ...
      "staff"     = "Staff: Shortage of staff",
      "incentive" = "Incentive: Lack of financial incentives to \nmotivate providers",
      "flow"      = "Flow: Complex flow from screening to \ngetting PM+, leading to loss of patients",
      "prefer"    = "Prefer: Patients prefer to deal with one \nprovider as opposed to many",
      "confi"     = "Confi: Fear of breach of confidentiality",
      "long"      = "Long: Lengthening of patient visits",
      "partner"   = "Partner: Lack of partner involvement",
      .default    = barrier
...
p_pm_barrier_all <- ggplot(stats_pm, aes(x = mean_importance, y = mean_addressability)) +
  geom_point(aes(color = barrier_detail, size = sd_consensus), shape = 1) +
  scale_x_continuous(breaks = 1:4, limits = c(1, 4)) +
  scale_y_continuous(breaks = 1:4, limits = c(1, 4)) +
  geom_hline(yintercept = cutoff_addressability_pm, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = cutoff_importance_pm, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_importance_pm, y = 4,
           label = paste0("Cutoff: ", round(cutoff_importance_pm, 2)),
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 1.2, y = cutoff_addressability_pm,
           label = paste0("Cutoff: ", round(cutoff_addressability_pm, 2)),
           vjust = -1, size = 3.5) +
  labs(
    title = "Panel A: Barriers",
    x = "Importance",
    y = "Addressability",
    color = "Barrier detail"
  ) +
...
pm_barrier_table <- stats_pm %>%
  left_join(axis_summary_pm, by = "barrier") %>%
  mutate(
    zone = case_when(
      mean_importance >= cutoff_importance_pm &
        mean_addressability >= cutoff_addressability_pm ~ "Priority (Go)",
      mean_importance >= cutoff_importance_pm &
        mean_addressability <  cutoff_addressability_pm ~ "Important but hard",
      mean_importance <  cutoff_importance_pm &
        mean_addressability >= cutoff_addressability_pm ~ "Easy but lower value",
      TRUE                                              ~ "Deprioritize"
    ),
    near_cutoff = abs(mean_importance - cutoff_importance_pm) < 0.05 |
                  abs(mean_addressability - cutoff_addressability_pm) < 0.05,
    barrier_label = gsub("\\s*\\n\\s*", " ", barrier_detail)
  ) %>%
  arrange(desc(mean_importance), desc(mean_addressability)) %>%
  mutate(rank = row_number()) %>%
...
```{r subgroup analysis_pm}
# 1) subgroup stats
stats_pm_sub <- long_data_pm %>%
  filter(!is.na(provider_type)) %>%
  group_by(provider_type, barrier, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
...
# 2) subgroup-specific median cutoffs (median of item means)
cutoffs_pm_sub <- stats_pm_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_importance     = median(mean_importance, na.rm = TRUE),
    cutoff_addressability = median(mean_addressability, na.rm = TRUE),
...
# 3) plot helper
plot_pm_barrier_subgroup <- function(pt_label) {

  df <- stats_pm_sub %>% filter(provider_type == pt_label)

  cx <- cutoffs_pm_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_importance)
  cy <- cutoffs_pm_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_addressability)
...
# 4) subgroup plots + combine
p_pm_barrier_lay <- plot_pm_barrier_subgroup("Lay providers")
p_pm_barrier_formal <- plot_pm_barrier_subgroup("Formally trained providers")

p_pm_barrier_subgroups <- ggarrange(
  p_pm_barrier_lay, p_pm_barrier_formal,
  ncol = 2, common.legend = TRUE, legend = "bottom"
)

p_pm_barrier_subgroups

# ggsave("Fig_pm_barriers_subgroups_1200px.png",
#         p_pm_barrier_subgroups,
#         dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
...
ipmh_pm_strategy <- ipmh_pm %>% 
  ...
  select(record_id, provider_type, matches("stra|diff"), ipmh_trt_strategy_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))

long_pm_strat <- ipmh_pm_strategy %>%
  pivot_longer(
    cols = -c(record_id, provider_type, ipmh_trt_strategy_other),
    names_to = c("type", "strategy"), 
    names_pattern = "ipmh_trt_(.*)_(.*)"
  ) %>% 
  mutate(
...
# median split of the plotted points
cutoff_pm_impact <- median(stats_pm_strat$mean_impact, na.rm = TRUE)
cutoff_pm_feasibility <- median(stats_pm_strat$mean_feasibility, na.rm = TRUE)

stats_pm_strat <- stats_pm_strat %>% 
  mutate(
    strategy_detail = dplyr::recode(
      strategy, 
      "traincli"  = "Traincli: Train clinicians on all IPMH processes",
      "clin"      = "Clin: Have clinicians in charge of PM+ \ninstead of lay providers",
      "stream"    = "Stream: Streamline the workflow at the facility",
      "incentive" = "Incentive: Provide financial incentives to \nmotivate providers",
      "chv"       = "CHV: Involve community health promoters",
      "partner"   = "Partner: Encourage male partner involvement",
      "info"      = "Info: Provide IPMH information and \nreassurance at the triage desk",
      .default    = strategy
...
p_pm_strategy_all <- ggplot(stats_pm_strat, aes(x = mean_impact, y = mean_feasibility)) +
  geom_point(aes(color = strategy_detail, size = sd_consensus), shape = 1) +
  scale_x_continuous(breaks = 1:5, limits = c(4, 5)) +
  scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
  geom_hline(yintercept = cutoff_pm_feasibility, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = cutoff_pm_impact, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_pm_impact, y = 5,
           label = paste0("Cutoff: ", round(cutoff_pm_impact, 2)),
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 4.1, y = cutoff_pm_feasibility,
           label = paste0("Cutoff: ", round(cutoff_pm_feasibility, 2)),
           vjust = -1, size = 3.5) +
  labs(
    title = "Panel B: Strategies",
    x = "Impact",
    y = "Feasibility",
    color = "Strategy detail"
  ) +
...
fig2 <- ggarrange(
  p_pm_barrier_all, p_pm_strategy_all,
  nrow = 2,
  common.legend = F,
  legend = "right"
)
...
# ggsave("Fig2_1200px.png",
#         fig2,
#         dpi=400, width = 8.5, height = 8.5, limitsize = F, bg = "white")

# -----------------------------
# Companion table: per-strategy ratings, spread, and zone (PM / treatment)
# -----------------------------

axis_summary_pm_strat <- long_pm_strat %>%
  filter(type %in% c("stra", "diff")) %>%
  group_by(strategy, type) %>%
  summarise(
    n      = sum(!is.na(value_1)),
...
pm_strategy_table <- stats_pm_strat %>%
  left_join(axis_summary_pm_strat, by = "strategy") %>%
  mutate(
    zone = case_when(
      mean_impact >= cutoff_pm_impact &
        mean_feasibility >= cutoff_pm_feasibility ~ "Priority (Go)",
      mean_impact >= cutoff_pm_impact &
        mean_feasibility <  cutoff_pm_feasibility ~ "High impact but hard",
      mean_impact <  cutoff_pm_impact &
        mean_feasibility >= cutoff_pm_feasibility ~ "Easy but lower impact",
      TRUE                                        ~ "Deprioritize"
    ),
    near_cutoff = abs(mean_impact - cutoff_pm_impact) < 0.05 |
                  abs(mean_feasibility - cutoff_pm_feasibility) < 0.05,
    strategy_label = gsub("\\s*\\n\\s*", " ", strategy_detail)
  ) %>%
  arrange(desc(mean_impact), desc(mean_feasibility)) %>%
  mutate(rank = row_number()) %>%
...
```{r subgroup analysis_pm_strategy}
# 1) subgroup stats
stats_pm_strat_sub <- long_pm_strat %>%
  filter(!is.na(provider_type)) %>%
  group_by(provider_type, strategy, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
  ...
  mutate(
    strategy_detail = dplyr::recode(
      ...
      "chv"       = "CHV: Involve community health promoters",
      "partner"   = "Partner: Encourage male partner involvement",
      "info"      = "Info: Provide IPMH information and reassurance at the triage desk",
      .default    = strategy
    )
  )

# 2) subgroup-specific median cutoffs (median of item means)
cutoffs_pm_strat_sub <- stats_pm_strat_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_impact      = median(mean_impact, na.rm = TRUE),
    cutoff_feasibility = median(mean_feasibility, na.rm = TRUE),
...
# 3) plot helper
plot_pm_strategy_subgroup <- function(pt_label) {

  df <- stats_pm_strat_sub %>% filter(provider_type == pt_label)

  cx <- cutoffs_pm_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_impact)
  cy <- cutoffs_pm_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_feasibility)
...
# 4) subgroup plots + combine
p_pm_strategy_lay <- plot_pm_strategy_subgroup("Lay providers")
p_pm_strategy_formal <- plot_pm_strategy_subgroup("Formally trained providers")

p_pm_strategy_subgroups <- ggarrange(
  p_pm_strategy_lay, p_pm_strategy_formal,
  ncol = 2, common.legend = TRUE, legend = "bottom"
)

p_pm_strategy_subgroups

ggsave("Fig_pm_strategies_subgroups_1200px.png",
        p_pm_strategy_subgroups,
        dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
```
...
ipmh_telepsy <- ipmh_hcw %>% select(record_id, provider_type, starts_with("ipmh_telepsy_"))

ipmh_telepsy_barrier <- ipmh_telepsy %>% 
  select(record_id, provider_type, matches("bar|add"), ipmh_telepsy_barrier_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))

long_telepsy <- ipmh_telepsy_barrier %>%
  pivot_longer(
    cols = -c(record_id, provider_type, ipmh_telepsy_barrier_other),
    names_to = c("type", "barrier"), 
...
# median split of the plotted points
cutoff_telepsy_importance <- median(stats_telepsy$mean_importance, na.rm = TRUE)
cutoff_telepsy_addressability <- median(stats_telepsy$mean_addressability, na.rm = TRUE)

stats_telepsy <- stats_telepsy %>% 
  mutate(
    barrier_detail = dplyr::recode(
      barrier, 
      "sche"     = "Sche: Complex scheduling process",
      "drug"     = "Drug: Inadequate or lack of access to \nthe medication prescribed",
      "setup"    = "Setup: User challenges in setting up \nand using telepsychiatry",
      "internet" = "Internet: Internet connectivity",
      ...
      "language" = "Language: Language barrier between \nwomen and psychiatrists",
      "confi"    = "Confi: Fear of breach of confidentiality",
      "physical" = "Physical: Clients prefer to talk to psychiatrists\n in person instead of online",
      .default   = barrier
    )
  )

p_telepsy_barrier_all <- ggplot(stats_telepsy, aes(x = mean_importance, y = mean_addressability)) +
  geom_point(aes(color = barrier_detail, size = sd_consensus), shape = 1) + 
  scale_x_continuous(breaks = 1:4, limits = c(1, 4)) +
  scale_y_continuous(breaks = 1:4, limits = c(1, 4)) +
  geom_hline(yintercept = cutoff_telepsy_addressability, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = cutoff_telepsy_importance, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_telepsy_importance, y = 4,
           label = paste0("Cutoff: ", round(cutoff_telepsy_importance, 2)),
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 1.2, y = cutoff_telepsy_addressability,
           label = paste0("Cutoff: ", round(cutoff_telepsy_addressability, 2)),
           vjust = -1, size = 3.5) +
  labs(
    title = "Panel A: Barriers",
    x = "Importance",
...
p_telepsy_barrier_all

# -----------------------------
# Companion table: per-barrier ratings, spread, and zone (telepsychiatry)
# -----------------------------

axis_summary_telepsy <- long_telepsy %>%
  filter(type %in% c("bar", "add")) %>%
  group_by(barrier, type) %>%
  summarise(
    n      = sum(!is.na(value_1)),
...
telepsy_barrier_table <- stats_telepsy %>%
  left_join(axis_summary_telepsy, by = "barrier") %>%
  mutate(
    zone = case_when(
      mean_importance >= cutoff_telepsy_importance &
        mean_addressability >= cutoff_telepsy_addressability ~ "Priority (Go)",
      mean_importance >= cutoff_telepsy_importance &
        mean_addressability <  cutoff_telepsy_addressability ~ "Important but hard",
      mean_importance <  cutoff_telepsy_importance &
        mean_addressability >= cutoff_telepsy_addressability ~ "Easy but lower value",
      TRUE                                                   ~ "Deprioritize"
    ),
    near_cutoff = abs(mean_importance - cutoff_telepsy_importance) < 0.05 |
                  abs(mean_addressability - cutoff_telepsy_addressability) < 0.05,
    barrier_label = gsub("\\s*\\n\\s*", " ", barrier_detail)
  ) %>%
  arrange(desc(mean_importance), desc(mean_addressability)) %>%
  mutate(rank = row_number()) %>%
...
telepsy_barrier_table
```

```{r subgroup analysis_telepsy}
# 1) subgroup stats
stats_telepsy_sub <- long_telepsy %>%
  filter(!is.na(provider_type)) %>%
  group_by(provider_type, barrier, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
...
# 2) subgroup-specific median cutoffs
cutoffs_telepsy_sub <- stats_telepsy_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_importance     = median(mean_importance, na.rm = TRUE),
    cutoff_addressability = median(mean_addressability, na.rm = TRUE),
...
# 3) plot helper
plot_telepsy_barrier_subgroup <- function(pt_label) {

  df <- stats_telepsy_sub %>% filter(provider_type == pt_label)

  cx <- cutoffs_telepsy_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_importance)
  cy <- cutoffs_telepsy_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_addressability)
...
# 4) subgroup plots + combine
p_telepsy_barrier_lay <- plot_telepsy_barrier_subgroup("Lay providers")
p_telepsy_barrier_formal <- plot_telepsy_barrier_subgroup("Formally trained providers")

p_telepsy_barrier_subgroups <- ggarrange(
  p_telepsy_barrier_lay, p_telepsy_barrier_formal,
  ncol = 2, common.legend = TRUE, legend = "bottom"
)

p_telepsy_barrier_subgroups

# ggsave("Fig_telepsy_barriers_subgroups_1200px.png",
#         p_telepsy_barrier_subgroups,
#         dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
```
...
ipmh_telepsy_strategy <- ipmh_telepsy %>% 
  select(record_id, provider_type, matches("stra|diff"), ipmh_telepsy_stra_other) %>%
  mutate(across(where(is.numeric), ~ na_if(., 99)))

long_telepsy_strat <- ipmh_telepsy_strategy %>%
  pivot_longer(
    cols = -c(record_id, provider_type, ipmh_telepsy_stra_other),
    names_to = c("type", "strategy"),
    names_pattern = "ipmh_telepsy_(.*)_(.*)"
  ) %>%
  mutate(
...
stats_telepsy_strat <- long_telepsy_strat %>%
  group_by(strategy, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
    sd_value   = sd(value_1, na.rm = TRUE),
...
# median split of the plotted points
cutoff_telepsy_impact <- median(stats_telepsy_strat$mean_impact, na.rm = TRUE)
cutoff_telepsy_feasibility <- median(stats_telepsy_strat$mean_feasibility, na.rm = TRUE)

stats_telepsy_strat <- stats_telepsy_strat %>% 
  mutate(
    strategy_detail = dplyr::recode(
      strategy, 
      "align"   = "Align: Align psychiatry appointments \nwith routine clinic visits",
      "mm"      = "MM: Have mentor mothers sit in \nduring telepsychiatry",
      "lang"    = "Lang: Involve translators for language \nbarrier or those who are deaf",
      "partner" = "Partner: Have a support partner \nduring telepsychiatry",
      "nurse"   = "Nurse: Involve nurses and clinicians \nduring telepsychiatry",
      .default  = strategy
    )
  )

p_telepsy_strategy_all <- ggplot(stats_telepsy_strat, aes(x = mean_impact, y = mean_feasibility)) +
  geom_point(aes(color = strategy_detail, size = sd_consensus), shape = 1) +
  scale_x_continuous(breaks = 1:5, limits = c(4, 5)) +
  scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
  geom_hline(yintercept = cutoff_telepsy_feasibility, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = cutoff_telepsy_impact, linetype = "dashed", color = "grey") +
  annotate("text", x = cutoff_telepsy_impact, y = 5,
           label = paste0("Cutoff: ", round(cutoff_telepsy_impact, 2)),
           hjust = -0.1, size = 3.5) +
  annotate("text", x = 4.1, y = cutoff_telepsy_feasibility,
           label = paste0("Cutoff: ", round(cutoff_telepsy_feasibility, 2)),
           vjust = -1, size = 3.5) +
  labs(
    title = "Panel B: Strategies",
    x = "Impact",
...
p_telepsy_strategy_all

fig3 <- ggarrange(
  p_telepsy_barrier_all, p_telepsy_strategy_all,
  nrow = 2,
  common.legend = F,
  legend = "right"
)
...
# ggsave("Fig3_1200px.png",
#         fig3,
#         dpi=400, width = 8.5, height = 8.5, limitsize = F, bg = "white")

# -----------------------------
# Companion table: per-strategy ratings, spread, and zone (telepsychiatry)
# -----------------------------

axis_summary_telepsy_strat <- long_telepsy_strat %>%
  filter(type %in% c("stra", "diff")) %>%
  group_by(strategy, type) %>%
  summarise(
    n      = sum(!is.na(value_1)),
...
telepsy_strategy_table <- stats_telepsy_strat %>%
  left_join(axis_summary_telepsy_strat, by = "strategy") %>%
  mutate(
    zone = case_when(
      mean_impact >= cutoff_telepsy_impact &
        mean_feasibility >= cutoff_telepsy_feasibility ~ "Priority (Go)",
      mean_impact >= cutoff_telepsy_impact &
        mean_feasibility <  cutoff_telepsy_feasibility ~ "High impact but hard",
      mean_impact <  cutoff_telepsy_impact &
        mean_feasibility >= cutoff_telepsy_feasibility ~ "Easy but lower impact",
      TRUE                                             ~ "Deprioritize"
    ),
    near_cutoff = abs(mean_impact - cutoff_telepsy_impact) < 0.05 |
                  abs(mean_feasibility - cutoff_telepsy_feasibility) < 0.05,
    strategy_label = gsub("\\s*\\n\\s*", " ", strategy_detail)
  ) %>%
  arrange(desc(mean_impact), desc(mean_feasibility)) %>%
  mutate(rank = row_number()) %>%
...
telepsy_strategy_table
```

```{r subgroup analysis_telepsy_strategy}
# 1) subgroup stats
stats_telepsy_strat_sub <- long_telepsy_strat %>%
  filter(!is.na(provider_type)) %>%
  group_by(provider_type, strategy, type) %>%
  summarise(
    mean_value = mean(value_1, na.rm = TRUE),
...
# 2) subgroup-specific median cutoffs (median of item means)
cutoffs_telepsy_strat_sub <- stats_telepsy_strat_sub %>%
  group_by(provider_type) %>%
  summarise(
    cutoff_impact      = median(mean_impact, na.rm = TRUE),
    cutoff_feasibility = median(mean_feasibility, na.rm = TRUE),
...
# 3) plot helper
plot_telepsy_strategy_subgroup <- function(pt_label) {

  df <- stats_telepsy_strat_sub %>% filter(provider_type == pt_label)

  cx <- cutoffs_telepsy_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_impact)
  cy <- cutoffs_telepsy_strat_sub %>% filter(provider_type == pt_label) %>% pull(cutoff_feasibility)

  ggplot(df, aes(x = mean_impact, y = mean_feasibility)) +
    geom_point(aes(color = strategy_detail, size = sd_consensus), shape = 1) +
    scale_x_continuous(breaks = c(4.5, 5), limits = c(4.5, 5)) +
    scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
    geom_hline(yintercept = cy, linetype = "dashed", color = "grey") +
    geom_vline(xintercept = cx, linetype = "dashed", color = "grey") +
    annotate("text", x = cx, y = 5,
...
# 4) subgroup plots + combine
p_telepsy_strategy_lay <- plot_telepsy_strategy_subgroup("Lay providers")
p_telepsy_strategy_formal <- plot_telepsy_strategy_subgroup("Formally trained providers")

p_telepsy_strategy_subgroups <- ggarrange(
  p_telepsy_strategy_lay, p_telepsy_strategy_formal,
  ncol = 2, common.legend = TRUE, legend = "bottom"
)

p_telepsy_strategy_subgroups

# ggsave("Fig_telepsy_strategies_subgroups_1200px.png",
#         p_telepsy_strategy_subgroups,
#         dpi=300, width = 12, height = 6, limitsize = F, bg = "white")
```
