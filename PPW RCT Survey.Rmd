...
##
scores <- pm_telep_df %>% 
    filter(clt_study_site == "17, Ramula Health Centre")
...
abs <- pm_survey_df %>% 
    filter(pm_facility == "Ramula Health Centre")
...
### Duplicated PM+ Surveys
duplicate_record <- pm_survey_df %>% 
    filter(pm_ptid == "21229510")
...
pm_session_wide %>% 
    tbl_summary(include = c(facility, `Pre-intervention PSYCHLOPS measurement`, `Session 1`, `Session 2`),
