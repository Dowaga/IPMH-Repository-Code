...
demographics_df %>% tbl_summary(
  ...
  as_gt() %>%
  # modify with gt functions
  gt::tab_header("Table 2: Basic Demographic Summary") %>% 
  gt::tab_options(
    table.font.size = "medium",
    data_row.padding = gt::px(1)) %>%
    tab_options(
...
pm_session_wide %>% 
    tbl_summary(include = c(facility, `Pre-intervention PSYCHLOPS measurement`, `Session 1 content`, `Session 2 content`, `Session 3 content`, `Session 4 content`),
...
session_wide %>% 
    tbl_summary(include = c(pm_facility, `Pre-intervention PSYCHLOPS measurement`, `Session 1 content`, `Session 2 content`, `Session 3 content`),
...
```{r PM+ Weekly Retention, message=FALSE, warning=FALSE, include=FALSE}
source("PM+ Follow-up.R")
V_week1 %>% 
    gt()
...
```{r Study Staffs PHQ9 and GAD7 Screening Yield, echo=FALSE, message=FALSE, warning=FALSE}
source
fc_yield_summary
