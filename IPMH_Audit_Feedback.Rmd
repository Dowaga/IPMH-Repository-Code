...
This script generates the audit and feedback report for each facility based on the calendar months. For the fifth audit and feedback session, we will report statistics from October 1st to December 31st. Data from the report should be abstracted for each facility and put into the slide template.

# 1. PHQ2 and GAD2 Screening Rate Per Facility (Per month & October - December Combined)
...
kable(total_screening_rate, 
      caption = "October - December Combined PHQ2/GAD2 Screening Rate by Facility",
      digits = 2)
```

\newpage

# 2. PHQ9 and GAD7 Screening Rate Among New Study Participants Per Facility (Per month & October - December Combined)
...
for (facility in facilities) {
  facility_data <- phq9_screening_weekly %>%
    filter(clt_study_site == facility)
...
kable(phq9_screening_monthly, 
      caption = "PHQ9/GAD7 Monthly Screening Rate by Facility (for new participants)",
      digits = 2)
```

```{r, total secondary screening rate table, echo = F, warning=F}
kable(phq9_screening_total, 
      caption = "October - December Combined PHQ9/GAD7 Screening Rate by Facility (for new participants)",
      digits = 2)
```

\newpage

# 3. PM+ Referral Rate Per Facility (Per month & October - December Combined)
...
kable(pm_referral_total,
      caption = "October - December Combined PM+ Referral Rate by Facility",
      digits = 2)
```

\newpage

# 4. Telepsychiatry Referral Rate Per Facility (Per month & October - December Combined)
...
kable(total_referral_summary_tele, caption = "October - December Combined Telepsychiatry Referral Rate by Facility",
      digits = 2)
```

\newpage

# 5. PM+ initiation rate per facility (Per month & October - December Combined)
...
kable(pm_initiation_all_merge_session1,
      caption = "October - December Combined PM+ Initiation Rate by Facility",
      digits = 2)
```
\newpage
...
# 7. Telepsychiatry initiation rate per facility (Per month & October - December Combined)
...
kable(telepsych_initiation_total,
      caption = "October - December Combined Telepsychiatry Initiation Rate by Facility",
      digits = 2)
```

\newpage

# 8. Frequency of conducting health talks & use of flipbook per facility (Per month & October - December Combined)

```{r health talks, echo = F, warning=F}
kable(health_talks_monthly, caption = "Monthly Health Talks & Flipbook Usage by Facility",
      digits = 2)
kable(total_health_talks, caption = "October - December Combined Health Talks & Flipbook Usage by Facility",
      digits = 2)
```
